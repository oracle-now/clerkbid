...
| Claim domain | `Claim` is Facebook-specific; statuses: primary → confirmed (→ Sale) or canceled/expired; backup → promoted → confirmed; `confirmClaim` is the **only Claim-domain transition** that creates a Sale; `claimService.ts` never touches Whatnot data. A separate Sale-creation path exists in `dataPorter.ts` `insertChildrenForEvent` (import restore). | `lib/services/claimService.ts`, `types/claim.ts`, `lib/services/dataPorter.ts` |
...
| DataPorter / export version | `EXPORT_VERSION = 7` (verified directly); `buildEventExport` queries `db.claims` and serializes all Claim rows as `ClaimExportShape[]`; `insertChildrenForEvent` remaps `legacyBidderId`, `legacyLotId`, `legacySaleId` and enforces reference integrity (missing Sale mapping throws a visible error, not silent null); `parseEventExportPayload` accepts v1–v7 (v1–v6 exports default `claims` to `undefined`, treated as empty) | `lib/services/dataPorter.ts` |
